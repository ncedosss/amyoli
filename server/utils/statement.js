...
function generateStatement(statementData) {
  ...
  doc.text("Account Statement", 120, 20, { align: "center" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255,255,255);
  doc.text("Account Summary", 112, 61);
  ...
  doc.text("Debit", 122, y + 6);
  doc.text("Credit", 152, y + 6);
  doc.text("Line Total", 168, y + 6);
...
y += rowHeight;
  ...
  const agingY = startY + tableHeight;
  doc.rect(14, startY, 181, tableHeight);
  const tableEndY = agingY; // stop exactly where gold section starts
  doc.line(35, startY, 35, tableEndY);
  doc.line(60, startY, 60, tableEndY);
  doc.line(120, startY, 120, tableEndY);
  doc.line(150, startY, 150, tableEndY);
  doc.line(165, startY, 165, tableEndY);  // credit end

  // ================= GOLD AGING SECTION =================

  doc.setFillColor(...gold);
  doc.rect(14, agingY, 181, 8, "F");

  doc.setTextColor(0,0,0);
  doc.setFontSize(9);

  doc.text("120 Days Overdue", 35, agingY + 6);
  doc.text("90 Days Overdue", 65, agingY + 6);
  doc.text("60 Days Overdue", 95, agingY + 6);
  doc.text("30 Days Overdue", 125, agingY + 6);
  doc.text("Due Now", 160, agingY + 6);

  const agingRowY = agingY + 8;
  doc.rect(14, agingRowY, 181, 8);

  doc.text("R", 35, agingRowY + 6);
  doc.text(aging.d120.toFixed(2), 55, agingRowY + 6, { align: "right" });

  doc.text("R", 65, agingRowY + 6);
  doc.text(aging.d90.toFixed(2), 85, agingRowY + 6, { align: "right" });

  doc.text("R", 95, agingRowY + 6);
  doc.text(aging.d60.toFixed(2), 115, agingRowY + 6, { align: "right" });

  doc.text("R", 125, agingRowY + 6);
  doc.text(aging.d30.toFixed(2), 145, agingRowY + 6, { align: "right" });

  doc.text("R", 160, agingRowY + 6);
  doc.text(aging.dueNow.toFixed(2), 193, agingRowY + 6, { align: "right" });

  doc.line(55, agingY, 55, agingRowY + 8);
  doc.line(85, agingY, 85, agingRowY + 8);
  doc.line(115, agingY, 115, agingRowY + 8);
  doc.line(145, agingY, 145, agingRowY + 8);
  doc.line(175, agingY, 175, agingRowY + 8);

  // ================= CURRENT BALANCE STRIP =================

  doc.setFillColor(...blue);
  const currentBalanceY = agingY + 16;

  doc.rect(14, currentBalanceY, 181, 10, "F");

  doc.setTextColor(255,255,255);
  doc.setFontSize(11);

  doc.text(
    `Account Current Balance  R  ${totalAmount.toFixed(2)}`,
    193,
    currentBalanceY + 7,
    { align: "right" }
  );
  ...
  const footerStartY = currentBalanceY + 15;

doc.text(
  `Your account balance is R${totalAmount.toFixed(2)}. Please make your payment to cover the balance by the due date.`,
  105,
  footerStartY,
  { align: "center", maxWidth: 180 }
);

doc.text("Make all checks payable to AMYOLI TOURS", 105, footerStartY + 7, { align: "center" });

doc.text("Thank you for your business!", 105, footerStartY + 14, { align: "center" });

doc.text(
  "Should you have any enquiries concerning this statement, please contact Mbulelo Mpikwa on 0782185358",
  105,
  footerStartY + 21,
  { align: "center", maxWidth: 180 }
);

doc.line(14, footerStartY + 25, 195, footerStartY + 25);

doc.text(
  "1688 Sundut Street, Atlantis, South Africa, Western Cape, 7349",
  105,
  footerStartY + 30,
  { align: "center" }
);
...
doc.text(
  "Tel: 0721968752 Fax: N/A E-mail: mpikwaM@amyolitours.co.za Web: N/A",
  105,
  footerStartY + 34,
  { align: "center" }
);
